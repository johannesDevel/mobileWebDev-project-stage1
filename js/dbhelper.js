...
class DBHelper {
  ...
  */
  static fetchRestaurants(callback) {
    if (navigator.onLine) {
      console.log('Restaurants are online');
      ...
      console.log('Restaurants are not online');
  ...
  static fillDatabase(restaurants) {
    console.log("saving restaurants to database:");
  ...
  */
  static mapMarkerForRestaurant(restaurant, map) {
    ...
    static fetchReviewsById(id, callback) {
      if (navigator.onLine) {
        ...
        idb.open('restaurants-reviews').then(function (db) {
          ...
          const currentReviews = reviews.filter(reviews => reviews.restaurant_id === id);
          // DBHelper.addReviewsFromOfflineDB(id, currentReviews, (offlineReviews) => {
          //   if (offlineReviews) {
    ...
    static addReviewsFromOfflineDB(id, reviews, callback) {
      idb.open('restaurants-reviews').then(function (db) {
        ...
        const newReviews = offlineReviews.filter(
          offlineReviews => offlineReviews.restaurant_id === id);
          const allReviews = reviews.concat(newReviews);
          console.log('offline reviews from database');
          ...
          idb.open('restaurants-reviews').then(function (db) {
            ...
            return response;
              ...
              offlineReviews.forEach(review => {
                DBHelper.sendReviewsToServer(review);
              });
            });
        }
